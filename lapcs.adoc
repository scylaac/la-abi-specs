...
by other instructions that move typed data across registers such as `movgr2fr`.
...
variants may resuit in linker errors or run-time failures.
...
store its declared type, but it can share an addressible byte with
...
* *GARs*: 8 general-purpose registers `$a0` - `$a7` for passing integral arguments,
where `$a0` and `$a1` are also used for returning integral values.

* *FARs*: 8 floating-point registers `$fa0` - `$fa7` for passing floating-point
arguments, where `$fa0` and `$fa1` are also used for returning FP values.

An argument is passed using the stack only when no appropriate argument register
is available.

Subroutines should ensure that the initial values of general-purpose registers
...
is only guaranteed to cover the `lp64d` variant, that is, `GRLEN == 64` and
`FRLEN == 64`.
...
* An integral or pointer argument is passed in a GAR if there is one available.
Otherwise, it is passed on the stack. In both cases, unsigned integer arguments
narrower than GRLEN bits are always zero-extended to GRLEN bits, and the
signed integer arguments are sign-extended.
...
* If the strcture contains only two fp64 members, or an fp64 member and
...
or pointer member, the floating-point member of the structure is passed in
the FAR, and the integral / pointer member is passed in the GAR. If no FAR
is availble, the members will be passed in two adjacent GARs separately.
...
If no GAR is availble, all of the structure is passed on the stack.
...
* The union is passed by reference and are replaced in the argument list with
...
And after a variadic argument has been passed on the stack, all future arguments
will also be passed on the stack, i.e., the last argument register may be left
...
on the stack by value. It should be noted that fp128 data type
is passed in an aligned GAR pair (the first register in the pair is
even-numbered).

*w~arg~ > 2 × GRLEN*

* It’s passed by reference and are replaced in the argument list with
the address. If there is an available GAR, the reference is passed in
the GAR, and passed on the stack if no GAR is available.

=== Returning

In general, `$a0` and `$a1` are used to return non-floating-point values,
while `$fa0` and `$fa1` are used to return floating-point values.

Values are returned in the same manner as a first named argument
of the same type would be passed. If such an argument would have
been passed by reference, the caller allocates memory for the return
value, and passes the address as an implicit first argument.

The reference of the return value is returned that is stored in
GAR `$a0` if the size of the return value is larger than 2×GRLEN bits.


[[C-data-types]]
== Appendix: C data types and machine data types

NOTE: For all base ABI types of LoongArch, the `char` datatype is
signed by default.
