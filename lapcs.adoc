...
Zero-length bitfields defined in C and {Cpp} should be ignored and take up
...
NOTE: In this case, C compilers ignore empty structures or unions while {Cpp}
...
an FAR if available. Note that in this case, members containing empty structures
or unions are ignored, even in {Cpp}, unless they have nontrivial copy
constructors or destructors. But there is an exception that non-zero-length
array of empty structures or unions are not ignored in {Cpp}.
